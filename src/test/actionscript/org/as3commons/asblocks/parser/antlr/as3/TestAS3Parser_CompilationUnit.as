...
public class TestAS3Parser_CompilationUnit extends TestAS3ParserBase
{
	[Test]
	public function test_compilationUnit():void
	{
		...
		assertEquals("(COMPILATION_UNIT (package (TYPE foo bar) (BLOCK ANNOTATIONS " +
				"(CLASS_DEF (MODIFIERS public) Baz TYPE_BLOCK))))", 
				tree(parser.compilationUnit()).toStringTree());	
		
		parser = ASTUtils.parse("package foo.bar { public class Baz extends Bar { } }");
		assertEquals("(COMPILATION_UNIT (package (TYPE foo bar) (BLOCK ANNOTATIONS " +
				"(CLASS_DEF (MODIFIERS public) Baz (extends (TYPE Bar)) TYPE_BLOCK))))", 
				tree(parser.compilationUnit()).toStringTree());
		
		parser = ASTUtils.parse("package foo.bar { public class Baz extends Bar " +
				"implements IPod, foo.bar.IBaz { } }");
		assertEquals("(COMPILATION_UNIT (package (TYPE foo bar) (BLOCK ANNOTATIONS " +
				"(CLASS_DEF (MODIFIERS public) Baz (extends (TYPE Bar)) (implements " +
		...
		parser = ASTUtils.parse("package foo.bar { public class Baz "
						+ "implements IPod, foo.bar.IBaz { } }");
		assertEquals("(COMPILATION_UNIT (package (TYPE foo bar) (BLOCK ANNOTATIONS " +
				"(CLASS_DEF (MODIFIERS public) Baz (implements (TYPE IPod) " +
				"(TYPE foo bar IBaz)) TYPE_BLOCK))))", 
				tree(parser.compilationUnit()).toStringTree());
		
		parser = ASTUtils.parse("package foo.bar { public interface IBaz { } }");
		assertEquals("(COMPILATION_UNIT (package (TYPE foo bar) (BLOCK ANNOTATIONS " +
				"(INTERFACE_DEF (MODIFIERS public) IBaz TYPE_BLOCK))))", 
				tree(parser.compilationUnit()).toStringTree());
		
		parser = ASTUtils.parse("package foo.bar { public interface IBaz " +
				"extends IGoo, IPod { } }");
		assertEquals("(COMPILATION_UNIT (package (TYPE foo bar) (BLOCK ANNOTATIONS " +
				"(INTERFACE_DEF (MODIFIERS public) IBaz (extends (TYPE IGoo) " +
				"(TYPE IPod)) TYPE_BLOCK))))", 
				tree(parser.compilationUnit()).toStringTree());
	}
	...
	public function test_mxmlScriptUnit():void
	{
		...
		assertEquals("(MXML_SCRIPT_UNIT (ANNOTATIONS (import (TYPE foo Bar))) " +
				"(VARIABLE_DEF (MODIFIERS private) var (VAR_DEC fox (: (TYPE Bax)) " +
				"(= (FUNCTION_CALL (new Bax) ARGUMENTS)))) ANNOTATIONS " +
				"(FUNCTION_DEF (MODIFIERS public) runTests PARAMS (: (TYPE void)) " +
				"(BLOCK (var (VAR_DEC foo (: (TYPE Bar)))))))", 
				tree(parser.mxmlScriptUnit()).toStringTree());
	}
	...
	public function test_packageBlockEntry():void
	{
		...
		assertEquals("(COMPILATION_UNIT (package (BLOCK ANNOTATIONS (VARIABLE_DEF " +
				"(MODIFIERS public) namespace (VAR_DEC outer_space (= \"http//www.universe.com/42\"))))))", 
				tree(parser.compilationUnit()).toStringTree());
		
		// CLASS_DEF
		parser = ASTUtils.parse("package { public class Foo { } } ");
		assertEquals("(COMPILATION_UNIT (package (BLOCK ANNOTATIONS (CLASS_DEF " +
				"(MODIFIERS public) Foo TYPE_BLOCK))))", 
				tree(parser.compilationUnit()).toStringTree());
		
		// INTERFACE_DEF
		parser = ASTUtils.parse("package { public interface IFoo { } } ");
		assertEquals("(COMPILATION_UNIT (package (BLOCK ANNOTATIONS (INTERFACE_DEF " +
				"(MODIFIERS public) IFoo TYPE_BLOCK))))", 
				tree(parser.compilationUnit()).toStringTree());
		
		// FUNCTION_DEF
		parser = ASTUtils.parse("package { public function packageFunction()" +
				":void { return; } } ");
		assertEquals("(COMPILATION_UNIT (package (BLOCK ANNOTATIONS (FUNCTION_DEF " +
				"(MODIFIERS public) packageFunction PARAMS (: (TYPE void)) " +
				"(BLOCK return)))))", 
				tree(parser.compilationUnit()).toStringTree());
	...
	public function test_annotations():void
	{
		...
		assertEquals("(ANNOTATIONS (METATAG Foo) (COMPILE_CONFIG (:: CONFIG " +
				"debug) (BLOCK (import (TYPE foo Bar)))) (include 'my.as') " +
				"(use namespace outer_space) (import (TYPE foo bar) *))", 
	...
	[Test]
	public function test_configIdent():void
	{
		// configIdent
		parser = ASTUtils.parse("CONFIG :: foo");
		assertEquals("(:: CONFIG foo)", 
				tree(parser.configIdent()).toStringTree());
	}
			
	[Test]
	public function test_configBlockDirective():void
	{
		// configBlockDirective
		parser = ASTUtils.parse("CONFIG :: foo { if (bar) { var foo:int = 42; } }");
		assertEquals("(COMPILE_CONFIG (:: CONFIG foo) (BLOCK (if (CONDITION bar) " +
				"(BLOCK (var (VAR_DEC foo (: (TYPE int)) (= 42)))))))", 
				tree(parser.configBlockDirective()).toStringTree());
	}
	...
	[Test]
	public function test_metaDirectiveList():void
	{
		// metaDirectiveList
		parser = ASTUtils.parse("(true, \"foo\", foo.bar.Baz, 42");
		assertEquals("(METATAG_PARAMS true \"foo\" (TYPE foo bar Baz) 42)", 
				tree(parser.metaDirectiveList()).toStringTree());
		
		parser = ASTUtils.parse("(arg1=true, arg2=\"foo\", arg3=foo.bar.Baz, arg4=42");
		assertEquals("(METATAG_PARAMS (= arg1 true) (= arg2 \"foo\") (= arg3 " +
				"(TYPE foo bar Baz)) (= arg4 42))", 
				tree(parser.metaDirectiveList()).toStringTree());
	}
						
	[Test]
	public function test_metaTagParam():void
	{
		// see test_metaDirectiveList()
	}
...
//				tree(parser.interfaceExtendsClause()).toStringTree());
	...
	[Test]
	public function test_namespaceDefinition():void
	{
		
	}
	
	[Test]
	public function test_functionDefinition():void
	{
		
	}
	
	[Test]
	public function test_variableDefinition():void
	{
	...
	public function test_typeBlock():void
	{
		...
		assertEquals("(TYPE_BLOCK (FIELD_DEF ANNOTATIONS (MODIFIERS public) " +
				"var (VAR_DEC foo (: (TYPE int)) (= 42))))", 
				tree(parser.classTypeBlock()).toStringTree());
		
		parser = ASTUtils.parse("{ protected function foo(arg:int):void { } }");
		assertEquals("(TYPE_BLOCK (METHOD_DEF ANNOTATIONS (MODIFIERS protected) " +
				"ACCESSOR_ROLE foo (PARAMS (PARAM arg (: (TYPE int)))) (: " +
				"(TYPE void)) BLOCK))", 
				tree(parser.classTypeBlock()).toStringTree());
		
		parser = ASTUtils.parse("{ protected namespace inner_space = \"http://mind.com\"; }");
		assertEquals("(TYPE_BLOCK (FIELD_DEF ANNOTATIONS (MODIFIERS protected) " +
				"namespace (VAR_DEC inner_space (= \"http://mind.com\"))))", 
				tree(parser.classTypeBlock()).toStringTree());
		
		parser = ASTUtils.parse("{[Foo]var i:int}");
		assertEquals("(TYPE_BLOCK (FIELD_DEF (ANNOTATIONS (METATAG Foo)) MODIFIERS " +
		...
		assertEquals("(TYPE_BLOCK (COMPILE_CONFIG (:: CONFIG debug) BLOCK))", 
				tree(parser.classTypeBlock()).toStringTree());		
		
		parser = ASTUtils.parse("{CONFIG::debug{public var debug:Boolean = true;}}");
		assertEquals("(TYPE_BLOCK (COMPILE_CONFIG (:: CONFIG debug) (BLOCK " +
				"(FIELD_DEF ANNOTATIONS (MODIFIERS public) var (VAR_DEC debug " +
				"(: (TYPE Boolean)) (= true))))))", 
				tree(parser.classTypeBlock()).toStringTree());		
	}

	[Test]
	public function test_typeBlockEntry():void
	{
		// see test_typeBlock()
	}
	
	[Test]
	public function test_configBlockEntry():void
	{
		// configBlockEntry
		parser = ASTUtils.parse("CONFIG :: foo { public var foo:int = 42; }");
		assertEquals("(COMPILE_CONFIG (:: CONFIG foo) (BLOCK (FIELD_DEF ANNOTATIONS " +
				"(MODIFIERS public) var (VAR_DEC foo (: (TYPE int)) (= 42)))))", 
				tree(parser.configBlockEntry()).toStringTree());
	}
	
	[Test]
	public function test_staticBlockEntry():void
	{
		// staticBlockEntry
		parser = ASTUtils.parse("{ var a:int = 0; }");
		assertEquals("(STATIC_BLOCK (var (VAR_DEC a (: (TYPE int)) (= 0))))", 
				tree(parser.staticBlockEntry()).toStringTree());
	}
	
	[Test]
	public function test_fieldDefinitionBlockEntry():void
	{
		parser = ASTUtils.parse("var a");
		assertEquals("(FIELD_DEF ANNOTATIONS MODIFIERS var (VAR_DEC a))", 
				tree(parser.classTypeBlockEntry()).toStringTree());
		...
		parser = ASTUtils.parse("public var a:int = 42");
		assertEquals("(FIELD_DEF ANNOTATIONS (MODIFIERS public) var (VAR_DEC a " +
				"(: (TYPE int)) (= 42)))", 
				tree(parser.classTypeBlockEntry()).toStringTree());
		
		parser = ASTUtils.parse("public static var a:int = 42");
		assertEquals("(FIELD_DEF ANNOTATIONS (MODIFIERS public static) var " +
				"(VAR_DEC a (: (TYPE int)) (= 42)))", 
				tree(parser.classTypeBlockEntry()).toStringTree());
		
		parser = ASTUtils.parse("public static const a:int = 42");
		assertEquals("(FIELD_DEF ANNOTATIONS (MODIFIERS public static) const " +
				"(VAR_DEC a (: (TYPE int)) (= 42)))", 
				tree(parser.classTypeBlockEntry()).toStringTree());
	}
	
	[Test]
	public function test_methodDefinitionBlockEntry():void
	{
		parser = ASTUtils.parse("function foo() {}");
		assertEquals("(METHOD_DEF ANNOTATIONS MODIFIERS ACCESSOR_ROLE foo " +
				"PARAMS BLOCK)", 
				tree(parser.classTypeBlockEntry()).toStringTree());
		
		parser = ASTUtils.parse("function foo():int {return -1;}");
		assertEquals("(METHOD_DEF ANNOTATIONS MODIFIERS ACCESSOR_ROLE foo " +
				"PARAMS (: (TYPE int)) (BLOCK (return (UNARY_MINUS 1))))", 
				tree(parser.classTypeBlockEntry()).toStringTree());
		
		parser = ASTUtils.parse("public function foo():int {return -1;}");
		assertEquals("(METHOD_DEF ANNOTATIONS (MODIFIERS public) ACCESSOR_ROLE " +
				"foo PARAMS (: (TYPE int)) (BLOCK (return (UNARY_MINUS 1))))", 
				tree(parser.classTypeBlockEntry()).toStringTree());
		
		parser = ASTUtils.parse("override public function foo(arg:int, arg1:int = 1, " +
				"...rest):int {return -1;}");
		assertEquals("(METHOD_DEF ANNOTATIONS (MODIFIERS override public) " +
				"ACCESSOR_ROLE foo (PARAMS (PARAM arg (: (TYPE int))) (PARAM arg1 " +
				"(: (TYPE int)) (= 1)) (PARAM ... rest)) (: (TYPE int)) (BLOCK " +
				"(return (UNARY_MINUS 1))))", 
				tree(parser.classTypeBlockEntry()).toStringTree());
	}
	
	[Test]
	public function test_optionalAccessorRole():void
	{

	}
	
	[Test]
	public function test_namespaceDefinitionBlockEntry():void
	{
	...
	[Test]
	public function test_reservedNamespace():void
	{
		parser = parse("public");
		// <PUBLIC>public</PUBLIC>
		assertEquals("public", tree(parser.reservedNamespace()).toStringTree());
		
		parser = parse("private");
		// <PRIVATE>private</PRIVATE>
		assertEquals("private", tree(parser.reservedNamespace()).toStringTree());
		
		parser = parse("protected");
		// <PROTECTED>protected</PROTECTED>
		assertEquals("protected", tree(parser.reservedNamespace()).toStringTree());
		
		parser = parse("internal");
		// <INTERNAL>internal</INTERNAL>
		assertEquals("internal", tree(parser.reservedNamespace()).toStringTree());		
	}
	
	[Test]
	public function test_typePostfixSyntax():void
	{
		parser = parse(".<Foo>");
		assertEquals("(TYPE_POSTFIX Foo)", 
				tree(parser.typePostfixSyntax()).toStringTree());
		
		parser = parse(".<Foo.<Bar>>");
		assertEquals("(TYPE_POSTFIX Foo (TYPE_POSTFIX Bar))", 
				tree(parser.typePostfixSyntax()).toStringTree());
		
		parser = parse(".< Foo .< Bar .< Baz > > >");
		assertEquals("(TYPE_POSTFIX Foo (TYPE_POSTFIX Bar (TYPE_POSTFIX Baz)))", 
				tree(parser.typePostfixSyntax()).toStringTree());
	}
	
	[Test]
	public function test_ident():void
	{
		parser = parse("foo");
		assertEquals("foo", tree(parser.ident()).toStringTree());
		
		parser = parse("xml");
		assertEquals("xml", tree(parser.ident()).toStringTree());
		
		parser = parse("use");
		assertEquals("use", tree(parser.ident()).toStringTree());
	}
	
	
	[Test]
	public function test_propertyDefinition():void
	{
		parser = ASTUtils.parse("public function get foo():Bar {return null;}");
		assertEquals("(METHOD_DEF ANNOTATIONS (MODIFIERS public) (ACCESSOR_ROLE get) " +
				"foo PARAMS (: (TYPE Bar)) (BLOCK (return null)))", 
				tree(parser.classTypeBlockEntry()).toStringTree());
		
		parser = ASTUtils.parse("public function set foo(value:Bar):void {}");
		assertEquals("(METHOD_DEF ANNOTATIONS (MODIFIERS public) (ACCESSOR_ROLE set) " +
				"foo (PARAMS (PARAM value (: (TYPE Bar)))) (: (TYPE void)) BLOCK)", 
				tree(parser.classTypeBlockEntry()).toStringTree());
	}
	...
	public function test_declarationStatement():void
	{
		...
		assertEquals("(var (VAR_DEC a (: (TYPE int))) (VAR_DEC b (: (TYPE String))) " +
				"(VAR_DEC c (: (TYPE Vector (TYPE_POSTFIX int)))))", 
				tree(parser.declarationStatement()).toStringTree());
	...
	public function test_forStatement():void
	{
		...
		// <FOR><FOR_INIT><VAR><VAR_DEC><IDENT>i</IDENT><TYPE_SPEC><TYPE><IDENT>
		// int</IDENT></TYPE></TYPE_SPEC><ASSIGN><DECIMAL_LITERAL>0</DECIMAL_LITERAL>
		// </ASSIGN></VAR_DEC></VAR></FOR_INIT><FOR_CONDITION><LT><IDENT>i</IDENT>
		// <IDENT>len</IDENT></LT></FOR_CONDITION><FOR_ITERATOR><POST_INC><IDENT>i
		// </IDENT></POST_INC></FOR_ITERATOR><BREAK>break</BREAK></FOR>
		...
		// <FOR><FOR_INIT><VAR><VAR_DEC><IDENT>i</IDENT><TYPE_SPEC><TYPE><IDENT>
		// ASTIterator</IDENT></TYPE></TYPE_SPEC><ASSIGN><FUNCTION_CALL><IDENT>
		// iterator</IDENT><ARGUMENTS>ARGUMENTS</ARGUMENTS></FUNCTION_CALL></ASSIGN>
		// </VAR_DEC></VAR></FOR_INIT><FOR_CONDITION><FUNCTION_CALL><FIELD_ACCESS><IDENT>
		// i</IDENT><IDENT>hasNext</IDENT></FIELD_ACCESS><ARGUMENTS>ARGUMENTS</ARGUMENTS>
		// </FUNCTION_CALL></FOR_CONDITION><FOR_ITERATOR>FOR_ITERATOR</FOR_ITERATOR>
		// <BREAK>break</BREAK></FOR>
		assertEquals("(for (FOR_INIT (var (VAR_DEC i (: (TYPE ASTIterator)) " +
				"(= (FUNCTION_CALL iterator ARGUMENTS))))) (FOR_CONDITION " +
				"(FUNCTION_CALL (FIELD_ACCESS i hasNext) ARGUMENTS)) " +
				"FOR_ITERATOR break)", 
				tree(parser.forStatement()).toStringTree());
	}
	...
	public function test_defaultXMLNamespaceStatement():void
	{
		...
		assertEquals("(DEFAULT_XML_NAMESPACE \"http://the.universe.com/420\")", 
	 ...
	 */
	 [Test]
	public function test_orExpression():void
	{
		parser = parse("5||6");
	 ...
	 * @throws RecognitionException 
	 */
	public function test_andExpression():void
	{
		parser = parse("5&&6");
	 ...
	 * @throws RecognitionException 
	 */
	public function test_bitwiseOrExpression():void
	{
		parser = parse("5|6");
	 ...
	 * @throws RecognitionException 
	 */
	public function test_bitwiseXorExpression():void
	{
		parser = parse("5^6");
	 ...
	 * @throws RecognitionException 
	 */
	public function test_bitwiseAndExpression():void
	{
		parser = parse("5&&6");
		// <LAND><DECIMAL_LITERAL>5</DECIMAL_LITERAL><DECIMAL_LITERAL>6</DECIMAL_LITERAL></LAND>
		assertEquals("(&& 5 6)", tree(parser.expression()).toStringTree());
	 ...
	 * @throws RecognitionException 
	 */
	public function test_equalityExpression():void
	{
		parser = parse("5==5");
	 ...
	 * @throws RecognitionException 
	 */
	public function test_relationalExpression():void
	{
	 ...
	 * @throws RecognitionException 
	 */
	public function test_shiftExpression():void
	{
		parser = parse("5<<6");
	 ...
	 * @throws RecognitionException 
	 */
	public function test_additiveExpression():void
	{
		parser = parse("5+6");
	 ...
	 * @throws RecognitionException 
	 */
	public function test_multiplicativeExpression():void
	{
		parser = parse("5*6");
	 ...
	 * @throws RecognitionException 
	 */
	public function test_unaryExpression():void
	{
		parser = parse("++i");
...
//			tree(parser.primaryExpression()).toStringTree());
	...
	public function test_qualifiedIdent():void
	{
		...
		assertEquals("(FUNCTION_CALL (:: (FIELD_ACCESS foo bar) bax) ARGUMENTS)", 
				tree(parser.expression()).toStringTree());
		...
		parser = parse("bar::baz[foo]::baz.bar()");
		assertEquals("(FUNCTION_CALL (FIELD_ACCESS (:: (ARRAY_ACCESS " +
				"(:: bar baz) foo) baz) bar) ARGUMENTS)", 
			tree(parser.expression()).toStringTree());		
	}
